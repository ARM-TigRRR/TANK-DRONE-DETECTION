...
tensorboard = SummaryWriter(
    log_dir=os.path.join(args.checkpoint_folder, "tensorboard", f"{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"))
...
if __name__ == '__main__':
    ...
    if args.validation_mean_ap:
        ...
        eval = MeanAPEvaluator(eval_dataset, net, arch=args.net,
                               eval_dir=os.path.join(args.checkpoint_folder, 'eval_results'))
        ...
        if epoch % args.validation_epochs == 0 or epoch == args.num_epochs - 1:
            ...
            if args.validation_mean_ap:
                ...
                for i in range(len(class_ap)):
                    tensorboard.add_scalar(f"Class Average Precision/{eval_dataset.class_names[i + 1]}", class_ap[i],
                                           epoch)
